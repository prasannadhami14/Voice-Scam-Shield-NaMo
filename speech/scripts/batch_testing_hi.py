...
def run_batch_test(audio_files_pattern: str, ground_truth_file: str, output_file: str = "results/hi/test_results.json"):
    """
    Run batch testing on audio files, compare with ground truth, and calculate accuracy/latency.

    Args:
        audio_files_pattern: Glob pattern for WAV files (e.g., "test_audio/*.wav").
    ...
    for wav_file in candidate_files:
        ...
        # Ground truth keys are relative to datasets/hi/ root
        rel_key = rel.replace("datasets/hi/", "")
    ...
    for wav_file in filtered_files:
        try:
            ...
            # audio is already float32 in [-1, 1] from sf.read(dtype="float32")
            
            # Process audio chunk
            chunk_start = time.time()
            result = transcriber.process_audio_chunk(audio)
            result["processing_time"] = time.time() - chunk_start
            # Store relative file key to match ground truth keys
            rel_cur = os.path.relpath(wav_file, base_dir).replace("datasets/hi/", "")
            result["file"] = rel_cur
...
if __name__ == "__main__":
    ...
    run_batch_test(
        audio_files_pattern="datasets/hi/test_audio_converted/*.wav",
        ground_truth_file="ground_truth/ground_truth.json",
        output_file="results/hi/test_results.json",
    )
