...
def run_batch_test(audio_files_pattern: str, ground_truth_file: str, output_file: str = "results/en/test_results_english.json"):
    """
    Run batch testing on audio files, compare with ground truth, and calculate accuracy/latency for English.

    Args:
        audio_files_pattern: Glob pattern for WAV files (e.g., "datasets/en/test_audio_english/*.wav").
    ...
    for wav_file in candidate_files:
        ...
        rel_key = rel.replace("datasets/en/", "")
    ...
    for wav_file in filtered_files:
        try:
            ...
            rel_cur = os.path.relpath(wav_file, base_dir).replace("datasets/en/", "")
            result["file"] = rel_cur
...
if __name__ == "__main__":
    ...
    run_batch_test(
        audio_files_pattern="datasets/en/test_audio_english/*.wav",
        ground_truth_file="ground_truth/ground_truth_english.json",
        output_file="results/en/test_results_english.json",
    )
