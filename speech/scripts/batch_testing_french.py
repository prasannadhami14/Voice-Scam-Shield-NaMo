...
def run_batch_test(audio_files_pattern: str, ground_truth_file: str, output_file: str = "results/fr/test_results_french.json"):
    """
    Run batch testing on audio files, compare with ground truth, and calculate accuracy/latency for French.

    Args:
        audio_files_pattern: Glob pattern for WAV files (e.g., "datasets/fr/test_audio_french/*.wav").
    ...
    for wav_file in candidate_files:
        ...
        rel_key = rel.replace("datasets/fr/", "")
    ...
    for wav_file in filtered_files:
        try:
            ...
            rel_cur = os.path.relpath(wav_file, base_dir).replace("datasets/fr/", "")
            result["file"] = rel_cur
...
if __name__ == "__main__":
    ...
    run_batch_test(
        audio_files_pattern="datasets/fr/test_audio_french/*.wav",
        ground_truth_file="ground_truth/ground_truth_french.json",
        output_file="results/fr/test_results_french.json",
    )
