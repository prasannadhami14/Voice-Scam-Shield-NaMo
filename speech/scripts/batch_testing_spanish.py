...
def run_batch_test(audio_files_pattern: str, ground_truth_file: str, output_file: str = "results/es/test_results_spanish.json"):
    """
    Run batch testing on audio files, compare with ground truth, and calculate accuracy/latency for Spanish.

    Args:
        audio_files_pattern: Glob pattern for WAV files (e.g., "test_audio_spanish/*.wav").
    ...
    for wav_file in candidate_files:
        ...
        rel_key = rel.replace("datasets/es/", "")
        # normalize nonscam naming to ground truth keys
        if "nonscam_" in rel_key:
            rel_key = rel_key.replace("nonscam_", "non_scam_")
    ...
    for wav_file in filtered_files:
        try:
            ...
            # audio already float32 in [-1, 1]
            
            # Process audio chunk
            chunk_start = time.time()
            result = transcriber.process_audio_chunk(audio)
            result["processing_time"] = time.time() - chunk_start
            rel_cur = os.path.relpath(wav_file, base_dir).replace("datasets/es/", "")
            if "nonscam_" in rel_cur:
                rel_cur = rel_cur.replace("nonscam_", "non_scam_")
            result["file"] = rel_cur
            ...
            results.append({
                "file": os.path.relpath(wav_file, base_dir) if os.path.isabs(wav_file) else wav_file,
                "transcript": "",
                "risk": 0,
                "label": "Safe",
    ...
    for result in results:
        ...
        # Map "Non-Scam" to "Safe" for comparison
        if actual == "Non-Scam":
            actual = "Safe"
...
if __name__ == "__main__":
    ...
    run_batch_test(
        audio_files_pattern="datasets/es/test_audio_spanish/*.wav",
        ground_truth_file="ground_truth/ground_truth_spanish.json",
        output_file="results/es/test_results_spanish.json",
    )
